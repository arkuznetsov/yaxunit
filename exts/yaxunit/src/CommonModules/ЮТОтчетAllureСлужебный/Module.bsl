...
Функция ОписаниеТеста(РезультатТеста, Набор, Модуль) Экспорт
	...
	// Для smoke тестов используем имя теста, так как поле Представление не существует
	// в структуре РезультатТеста для реальных smoke тестов
	Если ЭтоSmokeТест(Модуль) Тогда
		// Используем Имя теста, которое содержит представление теста
		Описание.name = РезультатТеста.Имя;
		Описание.fullName = РезультатТеста.Имя + ПредставлениеРежима;
	Иначе
		Описание.name = РезультатТеста.Имя;
