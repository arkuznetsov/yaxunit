...
Процедура ФикцияРеквизитов(Контекст, ИменаРеквизитов, ТолькоНезаполненные, Исключая = Ложь) Экспорт
	...
	// Преобразуем строку в массив имен реквизитов
	...
	// Создаем множество имен реквизитов для быстрого поиска
	МножествоИменРеквизитов = Новый Массив;
	Для Каждого ИмяРеквизита Из КоллекцияИменРеквизитов Цикл
		МножествоИменРеквизитов.Добавить(СокрЛП(ИмяРеквизита));
	КонецЦикла;
	
	// Получаем все реквизиты
	ВсеРеквизиты = Реквизиты(Контекст);
	
	// Проверяем наличие всех указанных реквизитов
	...
	Для Каждого Реквизит Из ВсеРеквизиты Цикл
		...
		// Пропускаем реквизит если:
		// - Для режима прямого указания: реквизит не в списке
		// - Для режима исключения: реквизит в списке исключений
		Если РеквизитВСписке = Исключая Тогда
			Продолжить;
		КонецЕсли;
		
		// Проверяем заполненность только если это требуется
