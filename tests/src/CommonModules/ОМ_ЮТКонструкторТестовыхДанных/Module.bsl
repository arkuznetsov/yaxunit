...
Процедура ФикцияНезаполненныхИсключая() Экспорт
	...
	// Создаем документ с одним заполненным реквизитом
	Данные = ЮТест.Данные().КонструкторОбъекта("Документы.ПриходТовара")
		.ФикцияНезаполненныхИсключая("Поставщик") // Исключаем только Поставщик из заполнения
		.ДанныеОбъекта();
	
	// Проверяем что все реквизиты кроме исключенных заполнены
	ЮТест.ОжидаетЧто(Данные)
		.НеИмеетСвойства("Поставщик")
		.Свойство("Склад").Заполнено()
		.Свойство("Валюта").Заполнено()
		.Свойство("Организация").Заполнено();
	
	// Проверяем работу с табличной частью
	Данные = ЮТест.Данные().КонструкторОбъекта("Документы.ПриходТовара")
		.ТабличнаяЧасть("Товары")
		.ДобавитьСтроку()
			.ФикцияНезаполненныхИсключая("Количество") // Исключаем только количество
		.ДанныеОбъекта();
	
	// Проверяем что все реквизиты строки кроме исключенных заполнены
	ЮТест.ОжидаетЧто(Данные)
		.НеИмеетСвойства("Товары[0].Количество")
