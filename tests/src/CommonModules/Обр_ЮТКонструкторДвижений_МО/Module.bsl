...
Процедура ИсполняемыеСценарии() Экспорт
	...
	ЮТТесты.ВТранзакции()
		...
		.ДобавитьТест("ФикцияНезаполненныхИсключая")
		...
		.ДобавитьТест("Записать_РегистрБухгалтерииССубконто")
	;
...
Процедура ФикцияРеквизитов() Экспорт
	...
	ЮТест.ОжидаетЧто(Конструктор)
		...
		.ВыбрасываетИсключение("`РегистрНакопления.ТоварныеЗапасы` не содержит реквизиты: `Несуществующий1`");
...
Процедура ФикцияНезаполненныхИсключая() Экспорт
	...
	// Добавляем строку, устанавливаем товар и заполняем все остальные реквизиты, кроме склада и количества
	Конструктор.ДобавитьСтроку()
		.Установить("Товар", Товар)
		.ФикцияНезаполненныхИсключая("Склад, Количество");
	
	// Получаем данные строки
	Данные = Конструктор.ДанныеСтроки();
	
	// Проверяем, что все реквизиты заполнены, кроме исключенных
	ЮТест.ОжидаетЧто(Данные, "Данные строки должны быть заполнены")
		.Заполнено()
		.Свойство("Товар")
			.Заполнено("Товар должен быть заполнен")
			.Равно(Товар)
		.Объект()
			.НеИмеетСвойства("Склад", "Склад не должен быть заполнен")
			.НеИмеетСвойства("Количество", "Количество не должно быть заполнено");
	
	// Проверяем, что при указании несуществующих реквизитов выбрасывается исключение
	МассивПараметров = ЮТКоллекции.ЗначениеВМассиве("Несуществующий1");
	ЮТест.ОжидаетЧто(Конструктор)
		.Метод("ФикцияНезаполненныхИсключая", МассивПараметров)
		.ВыбрасываетИсключение("`РегистрНакопления.ТоварныеЗапасы` не содержит реквизиты: `Несуществующий1`");
...
Процедура Записать_РегистрБухгалтерииССубконто() Экспорт
	
	// Создаем документ
	Документ = ЮТест.Данные().СоздатьДокумент("Документы.РасходТовара");
	КонтрагентСсылка = ЮТест.Данные().СоздатьДокумент("Справочники.Контрагенты");
	
	// Инициализируем конструктор движений
	ДанныеЗаписи = Новый Структура();
	ДанныеЗаписи.Вставить("Счет", ПредопределенноеЗначение("ПланСчетов.Основной.Товары"));
	ДанныеЗаписи.Вставить("ВидСубконто1", ПредопределенноеЗначение("ПланВидовХарактеристик.ВидыСубконто.Контрагент"));
	ДанныеЗаписи.Вставить("Субконто1", КонтрагентСсылка);
	ДанныеЗаписи.Вставить("Сумма", 10);
	ЮТест.Данные().КонструкторДвижений(Документ, "Основной")
		.ДобавитьСтроку(ДанныеЗаписи)
		.Записать();
		
	// Проверяем полученные движения документа
	ЮТест.ОжидаетЧтоТаблицаБазы("РегистрБухгалтерии.Основной")
		.СодержитЗаписи(ЮТест.Предикат()
			.Реквизит("Регистратор").Равно(Документ)
		);
	ЮТест.ОжидаетЧтоТаблицаБазы("РегистрБухгалтерии.Основной.Субконто")
		.СодержитЗаписи(ЮТест.Предикат()
			.Реквизит("Регистратор").Равно(Документ)
			.Реквизит("Вид").Равно(ДанныеЗаписи.ВидСубконто1)
			.Реквизит("Значение").Равно(ДанныеЗаписи.Субконто1)
		);
