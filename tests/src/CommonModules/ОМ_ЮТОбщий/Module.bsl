...
Процедура ИсполняемыеСценарии() Экспорт
	...
	ЮТТесты
		...
		.добавитьСерверныйТест("ПредставлениеЗначения_ТаблицаЗначений")
	;
...
Процедура ПредставлениеЗначения_ТаблицаЗначений() Экспорт
	
	ТаблицаЗначений = Новый ТаблицаЗначений;
	ТаблицаЗначений.Колонки.Добавить("Строка", Новый ОписаниеТипов("Строка"));
	...
	ТаблицаЗначений.Колонки.Добавить("Перечисление", Новый ОписаниеТипов("ПеречислениеСсылка.СостоянияЗаказов"));
	ТаблицаЗначений.Колонки.Добавить("СистемноеПеречисление", Новый ОписаниеТипов("ВидДвиженияНакопления"));
	ТаблицаЗначений.Колонки.Добавить("Справочник", Новый ОписаниеТипов("СправочникСсылка.Товары"));
	ТаблицаЗначений.Колонки.Добавить("Документ", Новый ОписаниеТипов("ДокументСсылка.ПриходТовара"));
	
	НаименованиеТовара = "Телевизор";
	Справочник = ЮТест.Данные().КонструкторОбъекта("Справочник.Товары")
		.Установить("Наименование", НаименованиеТовара)
		.Записать(, Истина);	
	ПредставлениеСправочника = СтрШаблон("%1 (%2)", НаименованиеТовара, XMLСтрока(Справочник));
	...
	Документ = ЮТест.Данные().КонструкторОбъекта("Документ.ПриходТовара")
		...
		.Записать(, Истина);	
	ПредставлениеДокумента = СтрШаблон("Поступление товара %1 от %2 (%3)", НомерДокумента, ДатаДокумента, XMLСтрока(Документ));
		
	НоваяСтрока = ТаблицаЗначений.Добавить();
	НоваяСтрока.Строка = "YaxUnit";
	НоваяСтрока.Число = 3.14;
	НоваяСтрока.Дата = '20250101';
	НоваяСтрока.Булево = Истина;
	НоваяСтрока.Перечисление = Перечисления.СостоянияЗаказов.ВРаботе;
	НоваяСтрока.СистемноеПеречисление = ВидДвиженияНакопления.Приход;
	НоваяСтрока.Справочник = Справочник;
	НоваяСтрока.Документ = Документ;
	
	ОжидаемоеЗначение = 
	"| Строка  | Число |        Дата        | Булево |    Перечисление    | СистемноеПеречисление |                    Справочник                    |                                      Документ                                      |
	||---------|-------|--------------------|--------|--------------------|-----------------------|--------------------------------------------------|------------------------------------------------------------------------------------|
	|| YaxUnit | 3.14  | 01.01.2025 0:00:00 |   Да   | В работе (ВРаботе) |        Приход         | " + 		ПредставлениеСправочника 		 + " | " + 							ПредставлениеДокумента 							  + " |";
	
	Результат = ЮТОбщий.ПредставлениеЗначения(ТаблицаЗначений);
